...
impl Sgl {
    ...
    pub fn set_painter_typeface(&mut self, object_id: Option<u64>) {
        unsafe {
            self.gpu
                .call_op(GpuOp::SetPainterTypeface { object_id })
                .unwrap();
        }
    }
    ...
    pub fn swap_painter_typeface(&mut self, object_id: Option<u64>) -> u64 {
        let old_typeface = self.get_painter_typeface();

        self.set_painter_typeface(object_id);
    ...
    pub fn mut_sgl() -> &'static mut Option<Sgl> {
        unsafe { &mut SGL }
    }

    pub fn sgl() -> &'static Option<Sgl> {
        unsafe { &SGL }
    }
}
